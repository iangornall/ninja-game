let setupCollisions = function(scene) {
  ...
  scene.enemies.forEach(enemy =>{
    scene.physics.add.collider(enemy, scene.map.staticLayers.main);
    scene.physics.add.collider(enemy, scene.map.staticLayers.main2);
    scene.physics.add.collider(enemy, scene.hero);
  })
}
